<template>
  <div class="form-container">
    ...
    <form v-if="selectedProduct !== null" @submit.prevent="deleteProduct">
    ...
    async deleteProduct() {
      ...
      try {
        ...
        if (response.status === 200) {
          // Make sure selectedProduct is not null before accessing it
          if (this.selectedProduct) {
            // Remove deleted product from the list
            this.products = this.products.filter(
              (product) => product.id !== this.selectedProduct?.id
            );
            this.selectedProduct = null; // Reset selected product after deletion
          }
        }
      } catch (error) {
